...
public class JavaStudy04
{
    ...
    public static void main(String[] args)
    {
        ...
        LocalDate end_day = YearMonth.from(LocalDate.parse(String.format("%s-%02d-01",year,month))).atEndOfMonth();
        int day = get_input("출생일을 입력해주세요.(dd):",1,Integer.parseInt(end_day.toString().substring(8,10)));

        int gender = get_input("성별을 입력해 주세요.(m/f):" , Integer.parseInt(year));

        /*yy+mm+dd - 성별+랜덤 6자리 숫자*/
        System.out.printf("%d%02d%02d-%d%06d",Integer.parseInt(year.substring(2,4)),month,day,gender,rand.nextInt(1000000));
    }
}